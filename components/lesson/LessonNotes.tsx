...
const components: PortableTextComponents = {
    block: {
      ...
      blockquote: ({ children }) => (
        <blockquote className="border-l-2 border-primary-500 pl-4 text-[15px] leading-7 text-neutral-700 italic">
          {children}
        </blockquote>
      ),
    },
    list: {
      bullet: ({ children }) => (
        <ul className="list-disc pl-5 text-[15px] leading-7 text-neutral-700 marker:text-primary-500">
          {children}
        </ul>
      ),
      number: ({ children }) => (
        <ol className="list-decimal pl-5 text-[15px] leading-7 text-neutral-700 marker:text-neutral-500">
          {children}
        </ol>
      ),
    },
...
const LessonNotes = ({notes}: {notes: BlockContent}) => {
  return (
