...
const COLLAPSED_MODULE_COUNT = 3;


const CourseContent = ({modules, durationSeconds}: CourseContentProps) => {
                    ...
                    <li key={key} className='border-b border-canvas-line last:border-b-0'>
                        ...
                        >
                            ...
                            <span className='relative  flex shrink-0 justify-center' aria-hidden>
                                <span className='flex size-7 items-center justify-center rounded-full border border-canvas-line bg-canvas text-sm leading-none text-neutral-700'>
                                    {moduleIndex +1}
                                </span>
                                ...
                                <span className='min-w-0 flex-1'>
                                    <span className='block font-display text-base leading-5 font-medium text-neutral-900'>
                                        {module.title}
                                    </span>
                                    {module.summary && (
                                ...
                                {moduleDuration && (
                                    <span className='shrink-0 text-sm leading-5 whitespace-nowrap text-neutral-500'>
                                        {moduleDuration}
                                    </span>
                                )}
                                ...
                                <ChevronDown
                                className={cn("size-4 shrink-0 text-neutral-500 transition-transform", isOpen && "rotate-180")}
                                strokeWidth={1.5}
                                aria-hidden
                                />
                            ...
                            {isOpen && (
                                <ul id={contentId} className='pb-3 pl-17 sm:pl-19'>
                                    {lessons.map((lesson, lessonIndex) => (
                                        <li key={lesson._id}>
                                            ...
                                            className="flex items-center gap-4 rounded-sm py-2 pr-14 text-neutral-700 transition-colors hover:text-primary-500! focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 sm:pr-16"
                                            >
                                            ...
                                            <div className='flex items-center gap-4 py-2 pr-14 sm:pr16'>
                                            <LessonRow
                                             label={lessonLabel(moduleIndex, lessonIndex)}
                                            title={lesson.title}
                                            duration={lesson.duration}
...
function LessonRow ({label, title, duration, freePreview}:LessonRowProps ) {
    ...
    return(
        <>
        <PlayCircle className='size-4 shrink-0 text-neutral-300' strokeWidth={1.5} aria-hidden />
        <span className='shrink-0 text-sm leading-5 text-neutral-500'>{label}</span>
        <span className='min-w-0 flex-1 text-sm leading-5'>{title}</span>
        {freePreview && (
            <Badge className='shrink-0 whitespace-nowrap bg-lesson-bg text-lesson-fg font-semibold'>
                Free Preview
            </Badge>
        )}
        {formatted && (
